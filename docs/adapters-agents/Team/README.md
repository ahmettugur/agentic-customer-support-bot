...
`PlanningAgent` (kendi `PlanningResult` şemasıyla) ve 4 specialist ajan (`SpecialistReasoningSchema` ile), `ChatOptions.ResponseFormat = ChatResponseFormat.ForJsonSchema<T>(...)` kullanarak OpenAI/Azure OpenAI `response_format=json_schema` (strict) özelliğini devreye sokar — model artık markdown fence unutamaz veya alan atlayamaz. Buna rağmen `PlanningResultParser`/`SpecialistReasoningParser`'ın defensive fence-temizleme + alan-bazlı parse mantığı **kaldırılmadı** — provider strict schema'yı herhangi bir nedenle honor etmezse parser yedek güvence olarak duruyor.
