...
- 🐞 **Dönüş tipi eskiden `List<StreamEvent>` idi.** "Olay yok" durumunda paylaşılan tek bir `NoEvents` örneği dönülüyordu (yeniden tahsis maliyetini önlemek için) — ama mutable `List<T>` dönüş tipi, bir çağıranın (bugün yok) bu örnek üzerinde `.Add(...)` çağırıp singleton'ı process ömrü boyunca kalıcı olarak bozmasını derleyicinin önlemesini engelliyordu. `NoEvents` artık `Array.Empty<StreamEvent>()`, dönüş tipi `IReadOnlyList<StreamEvent>` — bu yapısal olarak imkânsız.
