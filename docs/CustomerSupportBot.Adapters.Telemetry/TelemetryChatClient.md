...
`ILlmCallPersistencePort` opsiyonel bir bağımlılıktır (`sp.GetService<...>()` ile nullable alınır) — kayıtlı değilse `_persistence` null olur ve sadece OTLP + in-memory `CostUsageStore` çalışır. Pratikte `ILlmCallPersistencePort` her zaman `PostgresLlmCallUsageSink` olarak kayıtlıdır (bkz. [Persistence — Postgres vs InMemory](../CustomerSupportBot.Adapters.Persistence/README.md#postgres-vs-inmemory)); "InMemory provider" diye ayrı bir çalışma modu yoktur.
