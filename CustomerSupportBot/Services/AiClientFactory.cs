...
public static class AiClientFactory
{
    ...
    {
        AiProvider.AzureOpenAI => CreateAzureChatClient(options.AzureOpenAI, options.AzureOpenAI.Deployment),
        AiProvider.Anthropic => CreateAnthropicChatClient(options.Anthropic, options.Anthropic.Model),
        _ => CreateOpenAIChatClient(options.OpenAI, options.OpenAI.Model),
    };
    ...
    public static ReasoningChatClient CreateReasoningChatClient(AiOptions options)
    {
        switch (options.Provider)
        {
            case AiProvider.AzureOpenAI:
            {
                ...
                var deployment = !string.IsNullOrWhiteSpace(az.ReasoningDeployment) ? az.ReasoningDeployment! : az.Deployment;
                var client = CreateAzureChatClient(az, deployment);
                return new ReasoningChatClient(client, deployment, az.ReasoningEffort);
            }
            case AiProvider.Anthropic:
            {
                var an = options.Anthropic;
                var model = !string.IsNullOrWhiteSpace(an.ReasoningModel) ? an.ReasoningModel! : an.Model;
            ...
            default:
            {
                ...
                var model = !string.IsNullOrWhiteSpace(oa.ReasoningModel) ? oa.ReasoningModel! : oa.Model;
                var client = CreateOpenAIChatClient(oa, model);
                return new ReasoningChatClient(client, model, oa.ReasoningEffort);
            }
        }
    }
